...
class GameState:
    def __init__(self, screen):
        ...
        self.background = pygame.image.load(os.path.abspath(os.path.join(os.path.dirname(__file__), "../../assets/images/background6.png")))
        self.background = pygame.transform.scale(self.background, (SCREEN_WIDTH, SCREEN_HEIGHT))
        
        # load button pause image
    ...
    def handle_events(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            # check if the mouse position is within the slice object
            if self.pause_image_rect.collidepoint(mouse_pos):
                self.paused = not self.paused  # change state
    ...
    def draw(self):
        ...
        if self.pause_image_rect.collidepoint(mouse_pos):
            ...
            pygame.draw.rect(self.screen, PAUSE_IMAGE_HOVER_COLOR, self.pause_image_rect, 5)  
